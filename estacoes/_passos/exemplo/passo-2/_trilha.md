...
# Trilha — como esta estação funciona

## Parada 1 — Esta estação está vazia, e isso é o exercício

Nove capturas, nada mais. Sem notas, sem ideias, sem projetos.

A outra estação de exemplo (`exemplo-precos`) já tem tudo: cadeias inteiras,
...
**▶** e a estação passa para o estado seguinte.
...
noites da mesma pessoa, nasce a primeira **pendência**. Uma estação com um
projeto só não tem decisão nenhuma para tomar.

**Nada é calculado.** Cada passo é uma pasta com a estação inteira já naquele
estado, e o botão só a restaura. É um slideshow, não um motor.

Isso é honesto sobre o que a Central faz e o que ela não faz: numa estação
...
A pasta `_pendencias/` está vazia no passo 0, de propósito: uma estação com
...
Outro exemplo, de outro tipo, está em `estacoes/exemplo-precos/_pendencias/`:
...
- `estacoes/exemplo-precos` — a mesma estrutura, depois de meses de uso
