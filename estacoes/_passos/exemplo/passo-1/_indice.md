...
> **Esta estação é um exemplo, e ela começa vazia de propósito.** Só há
...
**É assim que uma estação de verdade começa:** cheia de matéria-prima e sem
...
Esta estação vem com uma **trilha**: sete estados sucessivos dela mesma,
seguindo dois grãos do começo ao fim. Clique em **▶** e a estação inteira
...
**Não há mágica nem inteligência aqui.** Cada passo é uma pasta com a estação
inteira já naquele estado, e o botão só a restaura. Numa estação sua a
...
**Para ver como fica uma estação depois de meses de uso**, abra
`estacoes/exemplo-precos` pelo seletor: lá tudo já aconteceu — duas cadeias
...
| `_trilha.md` | a leitura guiada desta estação (aba Tour) |
...
5. Para voltar ao começo, fora do app:
   `python ../../metodo/estacao.py reiniciar --raiz . --confirmar`

## Duas coisas que este exemplo simplifica

1. Numa estação de verdade, **cada projeto é um repositório git próprio**
   (ver `metodo/classificar.md`). Aqui os projetos que você criar vão viver
   dentro do repositório da estação, para o exemplo caber num clone só.
2. **A trilha e o reiniciar só existem aqui.** É a chave `estado_inicial` do
   `estacao.json` que os permite, e uma estação sua não a declara — é por
