...
Uma **estação de exemplo** da Central, sobre acompanhar preço de concorrente
e detectar lojas que copiam a sua. É a irmã de `estacoes/exemplo` (cozinha e
...
- **`_sem-destino.md` é gerado.** Não edite dentro dos marcadores
  `<!-- gerado:... -->`; rode
  `python ../../metodo/estacao.py gerar-sem-destino --raiz .`
- **Nunca monte identificador à mão:** `estacao.py novo-id`.
...
- Antes de encerrar: `python ../../metodo/estacao.py verificar --raiz .`

## O que este exemplo não segue

Numa estação de verdade cada projeto é um repositório git próprio. Aqui os
dois vivem dentro do repositório da estação, para caber num clone só. É a
